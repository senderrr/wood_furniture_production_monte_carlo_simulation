...
# columns for keeping track of whats made or not; wait time statistics
def priority_queue(pq_df, build_confidence, samples):
    print(pq_df)

    pq_df = pq_df.groupby('Day')['Item A', 'Item B', 'Item C', 'Item D', 'Item E'].sum()
    pq_df['Most Ordered Item'] = pq_df.max(axis=1)
    print(pq_df)
